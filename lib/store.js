...
const initial = { catalog: DEFAULT_CATALOG, shops: DEMO_SHOPS.map(x=>({...x,balance:0})), orders: [], ledger: [] };
...
export async function ensureShop(db, profile){
 ...
 if (shop) return shop;
 const newShop = { id: "shop-" + Date.now(), shopName: profile.shopName, phone: profile.phone, balance: 0 };
...
export function createOrder(db,{shop,items,total,note=""}){
 const order={
  id:"CMD-"+Date.now().toString().slice(-7), shopId:shop.id, shopName:shop.shopName, phone:shop.phone,
  items,total,note,status:"pending",paymentStatus:"unpaid",paidAmount:0,creditAmount:0,createdAt:Date.now()
 };
 ...
 if(typeof window!=="undefined"){
   fetch("/api/orders",{
     ...
     body:JSON.stringify({id:order.id,items,total,note}),
     keepalive:true,
   }).catch((error)=>console.error("order sync",error));
 }
 return order;
}
...
export function recordPayment(db,shopId,amount,note=""){
 ...
 return addLedger(db,{shopId,type:"payment",amount:-actual,note});
}
...
export function migrateLegacy(){
 ...
 db.orders=(db.orders||[]).map(o=>({
   ...o,shopId:o.shopId||("shop-"+String(o.phone||"").replace(/\D/g,"")),paymentStatus:o.paymentStatus||"unpaid",
