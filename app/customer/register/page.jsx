...
export default function CustomerRegister() {
  ...
  async function submit(e) {
    ...
    setBusy(true);
    try {
      const r = await fetch("/api/customer/register", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, name: form.name.trim() }) });
  ...
  return <main className="min-h-screen bg-[#f7f8f4] p-4 flex items-center justify-center">
    <form onSubmit={submit} className="w-full max-w-lg bg-white rounded-3xl p-6 shadow-sm border border-gray-200 space-y-4">
      ...
      <input required value={form.name} onChange={set("name")} placeholder="Your full name" className="w-full rounded-xl border p-3" autoComplete="name" />
      <input required value={form.phone} onChange={set("phone")} placeholder="Phone number (05/06/07...)" className="w-full rounded-xl border p-3" inputMode="tel" autoComplete="tel" />
      <input required minLength={8} type="password" value={form.password} onChange={set("password")} placeholder="Password (8+ characters)" className="w-full rounded-xl border p-3" autoComplete="new-password" />
      <input required minLength={8} type="password" value={form.confirmPassword} onChange={set("confirmPassword")} placeholder="Confirm password" className="w-full rounded-xl border p-3" autoComplete="new-password" />
